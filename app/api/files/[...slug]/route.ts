...
import { DeleteFileUseCase, DownloadFileUseCase } from "@/domains/file/file.usecase"
...
export async function GET(
  ...
  const { slug: slugArr } = await context.params // must await
  const slug = slugArr.join("/") // reconstruct full path
  ...
  try {
    if (list === "true") {
      ...
      return NextResponse.json(
        files.map(f => ({ url: `https://storage.pastelocity.com.my/public/${f.path}` }))
      )
    } else {
...
export async function DELETE(
  req: NextRequest,
  context: { params: Promise<{ slug: string[] }> } // must await
) {
  const { slug: slugArr } = await context.params
  const slug = slugArr.join("/")
  const { path } = await req.json()

  if (!path) return NextResponse.json({ error: "path is required" }, { status: 400 })
  if (!path.startsWith(`${slug}/`))
    return NextResponse.json({ error: "Invalid path for this collection" }, { status: 400 })
